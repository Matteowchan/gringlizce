...
print('[+] Hero: 2.400+ -> 2.500+ Kelime')

pat = re.compile(
    r'(class="qb-hub-card cat-kb-miktar-ve-derece".*?<span class="m-num">)100(</span><span class="m-lbl">Kelime</span>)',
    re.DOTALL
)
new_html, n = pat.subn(r'\g<1>200\g<2>', html)
if n:
    html = new_html
    print(f'[+] Miktar card: 100 -> 200')
else:
