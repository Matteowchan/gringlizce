...
var WB=(function(){
  ...
  function drawItem(it){
    ...
    else if(it.type==='carrow'){ var ex=it.x+it.w,ey=it.y+it.h,mx=it.x+it.w/2,my=it.y+it.h/2,len=Math.max(1,Math.sqrt(it.w*it.w+it.h*it.h)),px=-it.h/len,py=it.w/len,bend=(it.bend||0.3),cx=mx+px*len*bend,cy=my+py*len*bend; ctx.beginPath(); ctx.moveTo(it.x,it.y); ctx.quadraticCurveTo(cx,cy,ex,ey); ctx.stroke(); var a2=Math.atan2(ey-cy,ex-cx),L2=10+it.size; ctx.beginPath(); ctx.moveTo(ex,ey); ctx.lineTo(ex-L2*Math.cos(a2-0.4),ey-L2*Math.sin(a2-0.4)); ctx.moveTo(ex,ey); ctx.lineTo(ex-L2*Math.cos(a2+0.4),ey-L2*Math.sin(a2+0.4)); ctx.stroke(); }
  ...
  function redraw(){ if(!ctx)return; ctx.clearRect(0,0,W,H); ctx.fillStyle='#fbfaf6'; ctx.fillRect(0,0,W,H); items.forEach(drawItem); if(sel!=null&&sel>=0&&items[sel]){ var b=bbox(items[sel]); ctx.save(); ctx.strokeStyle='#2E6E6A'; ctx.setLineDash([6,4]); ctx.lineWidth=1.5; ctx.strokeRect(b.x-6,b.y-6,b.w+12,b.h+12); ctx.restore(); } fontLabel(); }
